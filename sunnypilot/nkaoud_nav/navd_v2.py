...
class NkaoudNavd:
  ...
  def _refresh_params(self) -> None:
    ...
    raw_tol = self.params.get("NkaoudNavTurnTolerance", return_default=True)
    self._turn_tolerance = float(raw_tol or 100) / 100.0
    raw_accel = self.params.get("NkaoudNavMaxLatAccel", return_default=True)
    self._max_lat_accel = float(raw_accel or 250) / 100.0  # stored as int * 100
