...
class DeveloperLayoutSP(DeveloperLayout):
  ...
  def _on_turn_signal_clicked(self, side: str):
    ...
    content = (
      ...
      f"{names[side]}{predicted} {tr('four times, 1 second each, with an off gap between each.')}</p>" +
    ...
    dialog = ConfirmDialog(content, tr("Run"), rich=True,
                           callback=lambda result: self._start_turn_signal(side) if result == DialogResult.CONFIRM else None)
    gui_app.push_widget(dialog)
